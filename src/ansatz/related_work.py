...
class OldAnsatz(Ansatz):
    U_params = 15
    pool_params = 2

    # TODO: work with num_classes > 2
    def __call__(self, params, dims_q, *_, **__):
        wires = range(sum(dims_q))
        while len(wires) > 1:
            convolution(U_SU4, params[: self.U_params], wires)
            wires = pooling(
                pooling_ansatz,
                params[self.U_params : self.U_params + self.pool_params],
                wires,
            )
            params = params[self.U_params + self.pool_params :]

        return wires  # np.array(wires)  # .item()

    # total_params = (15 + 2) * num_layers
    # TODO: work with num_classes > 2
    def total_params(self, dims_q, *_, **__):
        num_layers = int(np.ceil(np.log2(sum(dims_q))))
        return (self.U_params + self.pool_params) * num_layers
